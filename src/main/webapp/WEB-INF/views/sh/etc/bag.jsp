...
<div class="contentWrap">
		<div class="cate_"><span>Bags</span></div>
		<div class="content">
			<c:forEach items="${products}" var="b">
				<div class="item">
					...
					<div class="itemspan2">
						<fmt:formatNumber value="${b.p_price}" type="currency"
							currencySymbol="\\" />
					</div>
					<div class="sellerbtn">
						<c:if test="${loginAccount.a_userType == 2}">
							<button onclick="deleteProduct('${b.p_no}','${b.p_category}')">삭제 /</button>
							<button
								onclick="location.href='product.update.go?p_no=${b.p_no}'">수정</button>
						</c:if>
					</div>
				</div>
			</c:forEach>
